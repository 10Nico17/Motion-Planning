...
def rrt(environment, motionplanning, hyperparameters):
    ...
    while not finished or (time.time() - start_time) < timelimit:   
    #for i in range(2):
        ...
        if motionplanning['type'] == 'arm':
            ...
            #xnew, control = steer_car(xnear.config, xrand, dt, max_speed, max_steering_angle, k)
...
def main(input_file, output_file):
    ...
    if motionplanning['type'] == 'arm':
        ...
        path, actions = extract_path_with_controls(final_path_node)
        output_data = {
        'plan': {
            'type': 'car',
            'dt': motionplanning['dt'],
            ...
            'states': [list(map(float, state)) for state in path],
