...
def forward_dynamics_car(state, action, dt):
    ...
    theta_new = theta + (v / L_car) * np.tan(omega) * dt
...
def forward_dynamics_car(state, action, dt, L=3):
    #print('state: ', state)
    #print('action: ', action)
    x, y, theta = state
    s, phi = action
    ...
    # Simple bicycle model for forward dynamics
    x_new = x + s * np.cos(theta) * dt
    y_new = y + s * np.sin(theta) * dt
    theta_new = theta + (s / L) * np.tan(phi) * dt
    #print('np.array([x_new, y_new, theta_new]): ', np.array([x_new, y_new, theta_new]))
    return np.array([x_new, y_new, theta_new])
...
def steer_car(xnear, xrand, dt, min_speed, max_speed, min_steering, max_steering, k):
    ...
    xnew = forward_dynamics_car(xnear, [v, omega], dt)
    control=[v, omega]
    state = xnew.tolist()
...
def rrt(environment, motionplanning, hyperparameters):
    ...
    while (timelimit > 0 and (time.time() - start_time) < timelimit and not finished) or (timelimit == 0 and not finished):
        ...
        if motionplanning['type'] == 'arm':
            ...
            #print('#######xnear.config: ', xnear.config)
...
def main(input_file, output_file):
    ...
    #print('motionplanning: ', motionplanning)
    global L_car
    L_car = motionplanning['L']
